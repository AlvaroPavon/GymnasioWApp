...
import { prepareClassTypeImageAsset } from '../src/utils/classTypeImageUpload';
...
export default function DashboardScreen({ navigation }) {
  ...
  const activeFilterCount = (typeFilter !== 'all' ? 1 : 0) + (timeFilter !== 'all' ? 1 : 0);
  const shownClasses = useMemo(() => filterActivities({
    ...
    userId: me?.id,
    isClient
  }), [classes, selectedDate, activityView, typeFilter, timeFilter, me?.id, isClient]);
  ...
  const FiltersSheet = () => (
    ...
    >
      ...
      <Text style={styles.filterLabel}>Momento</Text>
      <View style={styles.filterOptions}>
        {[
          ['all', 'Todas'],
          ['upcoming', 'Próximas'],
          ...
          <Btn key={id} title={label} tone={timeFilter === id ? 'light' : 'gray'} onPress={() => setTimeFilter(id)} />
        ))}
      </View>
    ...
    <View>
      ...
      {loading ? (
        ...
        <ActivityMessage
          ...
          actionLabel={activityView === 'calendar' ? 'Volver a hoy' : undefined}
          onAction={activityView === 'calendar' ? () => setSelectedDate(new Date()) : undefined}
        />
      )}
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.inner, { paddingBottom: bottomInset + 34 }]}
        keyboardShouldPersistTaps="handled"
      ...
      {notice ? (
        <View pointerEvents="none" style={[styles.notice, notice.tone === 'error' && styles.noticeError, { bottom: bottomInset + 10 }]}>
...
const styles = StyleSheet.create({
  ...
  activitiesTop: { minHeight: 88, paddingTop: 12, paddingHorizontal: 20, paddingBottom: 12, backgroundColor: '#101010', flexDirection: 'row', alignItems: 'center', gap: 8 },
  activitiesIdentity: { flex: 1, minWidth: 0 },
  activitiesTitle: { color: '#f7f7f8', fontSize: 31, lineHeight: 36, fontWeight: '900', letterSpacing: -0.5 },
