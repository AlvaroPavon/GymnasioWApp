...
- **Mobile**: Expo SDK `57.0.16` + React Native `0.86.2` + React `19.2.3`, con nueva arquitectura.
...
│  ├─ src/services/{auth,push,reservation,token}.service.ts
...
└─ mobile/
   ...
   ├─ src/utils/{activityCalendar,privacyPolicy,classTypeImageUpload}.js
...
- **Usuarios**: `id`, `nombre`, `email`, `password_hash`, `rol`, `estado_mensualidad`, `membership_expires_at`, más `telefono`, `profile_picture` para la UI clonada.
...
- **Reservas**: `id`, `user_id`, `clase_id`, `estado`, `fecha_solicitud`.
...
9. Cron de recordatorio envía push a los `CONFIRMADA` a 45 minutos.
...
El esquema pedido no incluye log persistente de recordatorios. El cron usa una ventana de 45-46 minutos; para idempotencia fuerte agregá `NotificacionesEnviadas` o `recordatorio_enviado_at`.
