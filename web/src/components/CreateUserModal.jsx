...
export default function CreateUserModal({ isOpen, onClose, onUpdate }) {
  ...
  const handleClose = () => {
    stopCamera();
    setName(''); setEmail(''); setPassword(''); setPhone(''); setRole('CLIENT'); setFile(null);
    onClose();
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Error creando usuario. Revisa los campos.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="password" required value={password} onChange={e=>setPassword(e.target.value)} minLength="6" className="w-full bg-slate-800 border border-slate-700 p-3 rounded text-white" />
            </div>
