      ...
      <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
        ...
        >
          <button
            ...
            type="button"
            onClick={onClose}
            className="absolute right-4 top-4 z-50 rounded-full bg-black/80 px-3 py-2 text-xs font-bold text-white transition-colors hover:bg-black focus-visible:outline-2 focus-visible:outline-blue-400"
          >
            Cerrar
          </button>
          ...
          <div className="flex bg-black md:w-5/12 md:flex-col">
            {imageUrl && !imageFailed ? (
              ...
              <div
                ...
                className="relative flex h-48 w-2/5 items-center justify-center overflow-hidden border-r border-white/5 bg-gradient-to-br from-slate-800 via-slate-900 to-emerald-950/70 p-5 text-center md:h-60 md:w-full md:border-b md:border-r-0"
              >
                <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-emerald-400/10 blur-2xl" aria-hidden="true" />
                <div className="relative rounded-2xl border border-white/10 bg-slate-950/55 px-5 py-4 shadow-xl backdrop-blur-sm">
                  <span className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-xl border border-emerald-400/20 bg-emerald-400/10 text-emerald-300" aria-hidden="true">
                    <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8">
                      <path d="M4 10v4m3-7v10m10-10v10m3-7v4M7 12h10" strokeLinecap="round" />
                    </svg>
                  </span>
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Actividad</p>
                  <p className="mt-1 text-sm font-semibold text-slate-300">Imagen no disponible</p>
                </div>
              </div>
            )}
            <div className="flex-1 p-5">
              <span className="mb-3 inline-block rounded-full bg-blue-600 px-3 py-1 text-xs font-bold text-white">
              ...
              {cls.teacher && (
                <button type="button" onClick={() => onUserClick(cls.teacher)} className="flex items-center gap-3 rounded-lg text-left focus-visible:outline-2 focus-visible:outline-blue-400">
                  <span className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border-2 border-emerald-400 bg-slate-800">
                    {cls.teacher.profile_picture ? (
                      <img src={cls.teacher.profile_picture} alt="" className="h-full w-full object-cover" />
                    ) : (
                      <span className="font-bold text-emerald-300" aria-hidden="true">{cls.teacher.name?.charAt(0)}</span>
                    )}
                  </span>
                  <span>
