  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <button ref={closeButtonRef} type="button" onClick={onClose} className="absolute right-4 top-4 rounded-full bg-slate-800 px-3 py-2 text-xs font-bold text-slate-300 hover:text-white focus-visible:outline-2 focus-visible:outline-red-400">
            Cerrar
          </button>

          <h2 id="edit-class-title" className="mb-2 text-2xl font-bold text-red-300">Editar clase</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            {user.role === 'ADMIN' && (
              <label className="grid gap-1 text-xs text-slate-400 md:col-span-2">
                Profesor
                <select required value={teacherId} onChange={(event) => setTeacherId(event.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 p-3 text-slate-300 focus-visible:outline-2 focus-visible:outline-red-400">
            ...
            <label className="grid gap-1 text-xs text-slate-400 md:col-span-2">
              ...
              <input
                ...
                className="w-full rounded border border-slate-700 bg-slate-800 p-3 text-white focus-visible:outline-2 focus-visible:outline-red-400"
              />
            ...
            <label className="grid gap-1 text-xs text-slate-400">
              Inicio
              <input type="datetime-local" step="0.001" required value={startTime} onChange={(event) => setStartTime(event.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 p-3 text-white focus-visible:outline-2 focus-visible:outline-red-400" />
            </label>
            <label className="grid gap-1 text-xs text-slate-400">
              Fin
              <input type="datetime-local" step="0.001" required value={endTime} onChange={(event) => setEndTime(event.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 p-3 text-white focus-visible:outline-2 focus-visible:outline-red-400" />
            </label>
